            ...
            for step in tqdm(range(1, self.steps_per_episode + 1), desc="Steps", leave=False):
                ...
                for idx, agent_id in enumerate(agent_ids):
                    ...
                    agent.store_experience(
                        ...
                        neighbor_next_state=next_neighbor_states[idx],
                        neighbor_next_action=actions[idx],  # Assuming actions of neighbors are same for simplicity
                        done=dones[idx],
                        td_error=0.0  # Placeholder, should be calculated based on critic
                    )
